...
class RunBotRun():
    ...
    def levelup (self): #Increases the game level and then calls the level generator
        ...
        if self.lives < 3: #Gotta have some mercy
            self.lives += 1 
        if self.level == 41:    #Ends the game upon beating max level. 40 was the highest level I could do without the success rate of wall spawning plummeting beyond reasonable limits.
